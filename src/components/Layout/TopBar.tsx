...
export function TopBar() {
  ...
  return (
    <header className="h-14 flex items-center justify-between px-4 md:px-6 border-b border-border bg-surface-elevated flex-shrink-0">
      {/* Workspace selector */}
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="ghost"
            size="sm"
            className="gap-2 text-sm font-medium text-foreground hover:bg-muted h-8 px-3"
          >
            <div className="w-5 h-5 rounded bg-primary/20 flex items-center justify-center">
              <span className="text-[10px] font-bold text-primary">LE</span>
            </div>
            <span className="hidden sm:inline max-w-[140px] truncate">{workspace}</span>
