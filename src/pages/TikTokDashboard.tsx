...
export default function TikTokDashboard() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-5 max-w-screen-2xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 animate-fade-up">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-semibold text-foreground">TikTok Ads</h1>
          </div>
          <p className="text-sm text-muted-foreground mt-0.5">Loja Exemplo TK</p>
        </div>
      ...
      {/* KPI Cards */}
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 animate-fade-up" style={{ animationDelay: "80ms" }}>
        {[
          ...
          { title: "Conversões", value: tiktokKPIs.conversions.value.toLocaleString("pt-BR"), change: tiktokKPIs.conversions.change },
      ...
      <div className="bg-card border border-border rounded-xl p-4 animate-fade-up" style={{ animationDelay: "160ms" }}>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={chartData} margin={{ top: 4, right: 16, bottom: 4, left: 0 }}>
            ...
            <YAxis yAxisId="left" tick={{ fontSize: 10, fill: "hsl(215,16%,57%)" }} tickLine={false} axisLine={false} tickFormatter={(v) => `R$${(v / 1000).toFixed(0)}k`} />
            <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 10, fill: "hsl(215,16%,57%)" }} tickLine={false} axisLine={false} tickFormatter={(v) => `${v}x`} />
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border">
                {["Campanha","Objetivo","Status","Orçamento","Spend","Impressões","Cliques","CTR","CPM","Views","CPV","Conv.","ROAS"].map((h) => (
            ...
            <tbody className="divide-y divide-border">
              {paginated.map((c) => (
                <tr key={c.id} className="hover:bg-muted/30 transition-colors">
                  <td className="px-4 py-3 font-medium text-foreground whitespace-nowrap max-w-[160px] truncate">{c.name}</td>
                  <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{c.objective}</td>
                  <td className="px-4 py-3">{statusBadge(c.status)}</td>
                  <td className="px-4 py-3 text-muted-foreground metric-value whitespace-nowrap">R$ {c.budget.toLocaleString("pt-BR")}</td>
                  <td className="px-4 py-3 text-foreground metric-value whitespace-nowrap">R$ {c.spend.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}</td>
                  <td className="px-4 py-3 text-muted-foreground metric-value whitespace-nowrap">{c.impressions.toLocaleString("pt-BR")}</td>
                  <td className="px-4 py-3 text-muted-foreground metric-value whitespace-nowrap">{c.clicks.toLocaleString("pt-BR")}</td>
                  <td className="px-4 py-3 text-muted-foreground metric-value">{c.ctr}%</td>
                  <td className="px-4 py-3 text-muted-foreground metric-value whitespace-nowrap">R$ {c.cpm.toFixed(2)}</td>
                  <td className="px-4 py-3 text-muted-foreground metric-value whitespace-nowrap">{c.videoViews.toLocaleString("pt-BR")}</td>
                  <td className="px-4 py-3 text-muted-foreground metric-value whitespace-nowrap">{c.cpv > 0 ? `R$ ${c.cpv.toFixed(4)}` : "—"}</td>
                  <td className="px-4 py-3 text-muted-foreground metric-value">{c.conversions}</td>
                  <td className="px-4 py-3 metric-value whitespace-nowrap">
      ...
      {/* Video Performance */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-up" style={{ animationDelay: "240ms" }}>
        <div className="bg-card border border-border rounded-xl p-4">
          <h3 className="text-sm font-semibold text-foreground mb-3">Tempo Médio de Visualização (s)</h3>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={videoPerformance} layout="vertical" margin={{ top: 4, right: 16, bottom: 4, left: 0 }}>
              ...
              <XAxis type="number" tick={{ fontSize: 10, fill: "hsl(215,16%,57%)" }} tickLine={false} axisLine={false} unit="s" />
              <YAxis type="category" dataKey="name" tick={{ fontSize: 9, fill: "hsl(215,16%,57%)" }} tickLine={false} axisLine={false} width={100} />
        ...
        <div className="bg-card border border-border rounded-xl p-4">
          <h3 className="text-sm font-semibold text-foreground mb-3">View-Through Rate por Anúncio (%)</h3>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={videoPerformance} margin={{ top: 4, right: 4, bottom: 4, left: -20 }}>
              ...
              <YAxis tick={{ fontSize: 10, fill: "hsl(215,16%,57%)" }} tickLine={false} axisLine={false} unit="%" />
