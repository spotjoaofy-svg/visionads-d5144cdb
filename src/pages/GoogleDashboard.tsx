...
export default function GoogleDashboard() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-5 max-w-screen-2xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 animate-fade-up">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-semibold text-foreground">Google Ads</h1>
          </div>
          <p className="text-sm text-muted-foreground mt-0.5">Loja Exemplo</p>
        </div>
      ...
      {/* KPI Cards */}
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 animate-fade-up" style={{ animationDelay: "80ms" }}>
        {[
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border">
                {["Campanha","Tipo","Status","Orçamento","Spend","Impressões","Cliques","CTR","Conv.","CPA","ROAS"].map((h) => (
            ...
            <tbody className="divide-y divide-border">
              {paginated.map((c) => (
                <tr key={c.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3"><span className="text-xs bg-muted px-2 py-0.5 rounded text-muted-foreground">{c.type}</span></td>
                  ...
                  <td className="px-4 py-3 text-muted-foreground metric-value whitespace-nowrap">{c.ctr}%</td>
                  <td className="px-4 py-3 text-muted-foreground metric-value whitespace-nowrap">{c.conversions}</td>
                  <td className="px-4 py-3 text-muted-foreground metric-value whitespace-nowrap">{c.cpa > 0 ? `R$ ${c.cpa.toFixed(2)}` : "—"}</td>
        ...
        {pageCount > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-border">
            ...
            <div className="flex gap-1">
              ...
              <Button variant="outline" size="sm" className="h-7 text-xs" disabled={page === pageCount} onClick={() => setPage((p) => p + 1)}>Próxima</Button>
            </div>
          </div>
        )}
      </div>
              ...
              <tbody className="divide-y divide-border">
                {searchTerms.map((s, i) => (
                  <tr key={i} className="hover:bg-muted/30">
                    <td className="px-3 py-2.5 text-foreground max-w-[140px] truncate">{s.term}</td>
        ...
        <div className="bg-card border border-border rounded-xl p-4">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={qualityScoreData} margin={{ top: 4, right: 4, bottom: 4, left: -20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(240,10%,20%)" />
              <XAxis dataKey="score" tick={{ fontSize: 10, fill: "hsl(215,16%,57%)" }} tickLine={false} axisLine={false} />
...
// Need to import Cell for the bar chart
