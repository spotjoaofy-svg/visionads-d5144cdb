...
function AiScore({ score }: { score: number }) {
  ...
  return (
    <div className={cn("w-9 h-9 sm:w-10 sm:h-10 rounded-full border-2 flex items-center justify-center flex-shrink-0", bg)}>
...
function PlatformBadge({ platform }: { platform: string }) {
  ...
  return (
    <span className={cn("text-[10px] px-1.5 py-0.5 rounded border font-medium whitespace-nowrap", cfg.cls)}>
      {cfg.label}
    </span>
  );
}
    ...
    <div className="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-5 w-full min-w-0">
      ...
      <div className="animate-fade-up">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-lg sm:text-xl font-semibold text-foreground">Rankings de Desempenho</h1>
        </div>
        <p className="text-xs sm:text-sm text-muted-foreground">
          Pontuação por IA com base nos dados das suas contas
        </p>
      </div>
      ...
      <div className="flex gap-1 bg-muted p-1 rounded-lg w-fit animate-fade-up" style={{ animationDelay: "80ms" }}>
        {(["creatives", "campaigns"] as const).map((t) => (
          <button
            ...
            onClick={() => setTab(t)}
            className={cn(
              "text-xs sm:text-sm px-3 sm:px-4 py-1.5 rounded-md transition-all font-medium whitespace-nowrap",
        ...
        <div className="space-y-4 animate-fade-up" style={{ animationDelay: "120ms" }}>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
            <div className="flex items-center gap-1 overflow-x-auto pb-0.5 no-scrollbar">
              <span className="text-xs text-muted-foreground whitespace-nowrap flex-shrink-0">Plataforma:</span>
              {PLATFORM_FILTER.map((p) => (
                <button key={p} onClick={() => setPlatformFilter(p)}
                  className={cn("text-xs px-2.5 py-1 rounded-md border transition-all whitespace-nowrap flex-shrink-0",
            ...
            <div className="flex items-center gap-1 overflow-x-auto pb-0.5 no-scrollbar">
              <span className="text-xs text-muted-foreground whitespace-nowrap flex-shrink-0">Formato:</span>
              {FORMAT_FILTER.map((f) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            {/* Winning Patterns */}
            <div className="bg-card border border-border rounded-xl p-3 sm:p-4 h-fit">
              ...
              <div className="space-y-2.5">
                {[
                  ...
                  <div key={p.label} className="p-2.5 sm:p-3 bg-muted/40 rounded-lg">
                    <div className="text-[10px] text-muted-foreground uppercase tracking-wider">{p.label}</div>
                    <div className="text-xs sm:text-sm font-semibold text-foreground mt-0.5">{p.value}</div>
                    <div className="text-xs text-success mt-0.5">{p.detail}</div>
                  </div>
                ))}

                <div className="p-2.5 sm:p-3 bg-muted/40 rounded-lg">
              ...
              <tbody className="divide-y divide-border">
                {campaignsRanking.map((c) => (
                  <tr key={c.rank} className="hover:bg-muted/30 transition-colors">
                    <td className="px-3 py-3"><RankBadge rank={c.rank} /></td>
                    ...
                    <td className="px-3 py-3 metric-value text-foreground whitespace-nowrap">R$ {c.spend.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}</td>
                    ...
                    <td className="px-3 py-3 metric-value text-muted-foreground">{c.conversions}</td>
                    <td className="px-3 py-3">
                      <div className="flex items-center gap-1.5">
                        <div className="w-14 sm:w-20 h-1.5 bg-muted rounded-full overflow-hidden flex-shrink-0">
                          <div
